...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation();
    // If the product has options/flavors, we must navigate them to the details page to select it
  ...
  return (
    <div className="product-card">
      ...
      {/* NavLink wrap around info for clean product page routing */}
      <NavLink 
      ...
      <div className="product-purchase-row">
        ...
        <div style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
          ...
          <button 
            ...
            title={product.variants && product.variants.length > 0 ? "Select options" : "Add to cart"}
            aria-label={product.variants && product.variants.length > 0 ? "Select options" : "Add to cart"}
          >
            {product.variants && product.variants.length > 0 ? (
              // Options/variants indicator icon
